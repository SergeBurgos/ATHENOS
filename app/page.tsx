...
export default function Home() {
  ...
  const processFiles = async (fileList: FileList | File[]) => {
    ...
    for (const file of incoming) {
      if (file.type === 'application/pdf' && !isAdmin) {
        showToast("Los PDFs están disponibles en el plan Strategist. Las imágenes son gratis.");
        continue;
      }
      if (!ALLOWED_TYPES.includes(file.type)) {
        showToast(`Tipo no soportado: ${file.name}. Solo JPG, PNG, WEBP${isAdmin ? ' y PDF' : ''}.`);
        continue;
      }
      if (file.size > MAX_FILE_SIZE) {
        showToast(`${file.name} supera 5 MB.`);
        continue;
      }
      ...
      setAttachedFiles(prev => {
        if (prev.length >= MAX_FILES) {
          showToast('Máximo 4 archivos por mensaje.');
          return prev;
        }
        return [...prev, {
          name: file.name,
  ...
  const sendMessage = async (text?: string) => {
    ...
    try {
      ...
      if (response.status === 400) {
        ...
        showToast(errData.message || 'Error con el archivo.');
        setLoading(false);
        return;
      }
      ...
      if (response.status === 403) {
        const errData = await response.json();
        if (errData.error === 'PDF_REQUIRES_UPGRADE') {
          showToast(errData.message || 'Esta función requiere un plan pago.');
          return;
        }
  ...
  return (
    <div style={{ display: 'flex', width: '100vw', height: '100vh', overflow: 'hidden' }}>
      {toast && (
        <div className="athenos-toast" role="alert">
          {toast}
        </div>
      )}
              ...
              <div className="input-toolbar">
                <div className="it-left">
                  <input
                    ref={fileInputRef}
                    type="file"
                    accept={isAdmin ? "image/jpeg,image/png,image/webp,application/pdf" : "image/jpeg,image/png,image/webp"}
                    multiple
                    style={{ display: 'none' }}
                    onChange={(e) => {
